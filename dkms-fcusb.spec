%define module fcusb
%define version 3.11.04
%define card "AVM GmbH|FritzCard USB ISDN TA"
...
Source3: dkms-fcusb-remove-DECLARE_MUTEX_LOCKED.patch
Source4: dkms-fcusb-update-INIT_WORK-changes.patch
Source5: dkms-fcusb-update-usb_complete_t-functions.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fcusb-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fcusb-remove-usb_driver_owner.patch"
PATCH_MATCH[1]="^2\.6\.(1[6-9])|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fcusb-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[2]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fcusb-update-INIT_WORK-changes.patch"
PATCH_MATCH[3]="^2\.6\.([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fcusb-update-usb_complete_t-functions.patch"
PATCH_MATCH[4]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fcusb-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fcusb-remove-usb_driver_owner.patch \
         %{_sourcedir}/dkms-fcusb-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fcusb-update-INIT_WORK-changes.patch \
         %{_sourcedir}/dkms-fcusb-update-usb_complete_t-functions.patch;
do
